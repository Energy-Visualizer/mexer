...
def new_email_code(account_info: SignupForm) -> str:
    ...
    EmailAuthCode(
        code=code, account=new_user
    ).save()  # save account to database for verification later
    return code
...
def get_plot_title(query: ShapedQuery, exclude: list[str] | None = None) -> str:
    ...
    information = {
        key: val for key, val in query.items() if exclude is None or key not in exclude
    }
    ...
    # turning to year off
    if title_to_year == title_from_year:
        title_to_year = False
    ...
    title_components = [
        ...
        # year(s)
        ("in " + title_from_year if title_from_year else "")
        + ("-" + title_to_year if title_to_year else ""),
        # collection method
        (
            "collected by " + str(information.get("method"))
            if information.get("method")
            else ""
        ),
        # energy type
        (
            "for " + str(information.get("energy_type"))
            if information.get("energy_type")
            else ""
        ),
        # last stage
        (
        ...
        (
            "where industry aggregated by "
            + str(information.get("industry_aggregation"))
            if information.get("industry_aggregation")
            else ""
        ),
        # product aggregation
        (
            "and product aggregated by " + str(information.get("product_aggregation"))
            if information.get("product_aggregation")
            else ""
        ),
    ]
