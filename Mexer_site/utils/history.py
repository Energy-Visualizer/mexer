...
def get_user_history(request) -> list:
    ...
    if serialized_data:
        user_history = pickle.loads(
            bytes.fromhex(serialized_data)
...
def update_user_history(request, plot_type, query):
    ...
    history_data = {"plot_type": plot_type, **query}
    ...
    if user_history:
        ...
        try:
            ...
            pass  # don't care if not in, trying to remove anyways

        user_history.insert(
            0, history_data
        )  # finally, add the new query to the top of the history
