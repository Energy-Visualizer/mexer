...
def allow_iea(modeladmin, request, queryset):
    ...
    if GET_IEA_PERMISSION is None:
        GET_IEA_PERMISSION = Permission.objects.get(codename="get_iea")
...
class IEAAdmin(UserAdmin):
    ...
    actions = [
        allow_iea,
        remove_iea,
    ]  # this needs to be a list or there will be an error when Django looks for allow_iea()
    list_display = (
        "username",
        "iea_approved",
        "email",
    ...
    @display(boolean=True, description="IEA Approved")
