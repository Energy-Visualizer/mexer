...
from utils.history import get_history_html, get_user_history
...
def delete_history_item(request):
    ...
    if request.method == "POST":
        ...
        if index < 0:
            return error_pages.error_400(request, "Bad history index")
        ...
        if 0 > index >= len(user_history):
            return error_pages.error_400(
                request, "Bad history index for user history received"
            )
        ...
        response.set_cookie(
            "user_history", serialized_data.hex(), max_age=30 * 24 * 60 * 60
        )

        return response

    # Return an error response if the request is invalid
    return error_pages.error_400(request, "")
